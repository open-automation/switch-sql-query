...
var odbcConnect = function(s : Switch, job : Job, ds_name, username, password, callback){
	...
	if(data_source_connected == true){
		...
		job.fail("Could not connect to data source. Please check ODBC connection or credentials.");
	}
}
...
var getQueryResult = function(s : Switch, job : Job, data_source, query, result_type, callback){
	...
	if(query_successful == true){
		...
		job.fail("Query execution was not successful.");
	}
}
...
function jobArrived( s : Switch, job : Job )
{
	...
	if(query_type == 'Parameterized'){
		...
		if(query === false){
			// Fail job
			job.fail("Query has failed to parameterize. See previous log message(s).");
			return false;
		} else {
			s.log(-1, query);
		}
